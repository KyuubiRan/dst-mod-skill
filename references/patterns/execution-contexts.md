...
Read `references/patterns/entrypoint-patterns.md` when the real question is "which root file should own this change?"

When the task is about rooms, tasks, levels, presets, or start locations, read `references/patterns/worldgen-patterns.md` even if the mod is already classified as client-only, server-only, or all-clients gameplay.
