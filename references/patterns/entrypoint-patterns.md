...
If the task is mainly about compatibility flags or `configuration_options`, also read `references/patterns/modinfo-patterns.md`.
If the task is mainly about `modmain.lua` as the runtime registration hub, also read `references/patterns/modmain-patterns.md`.
If the task is mainly about rooms, tasks, presets, or start locations, also read `references/patterns/worldgen-patterns.md`.
