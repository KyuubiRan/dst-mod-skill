...
Read `references/patterns/runtime-i18n-patterns.md` too when the task is about the overall runtime i18n architecture rather than one or two direct `STRINGS` writes.
Read `references/patterns/modinfo-patterns.md` instead when the text belongs in `modinfo.lua` metadata or config UI.
...
Use a locale loader plus sparse locale tables as described in `references/patterns/runtime-i18n-patterns.md`.
...
For broader `modinfo.lua` rules such as config grouping and environment limits, also read `references/patterns/modinfo-patterns.md`.
