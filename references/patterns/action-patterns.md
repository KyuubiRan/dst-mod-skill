...
If the task also needs deeper SG patching, prediction, or `wilson` versus `wilson_client` routing, also read `references/patterns/stategraph-patterns.md`.
If the interaction is purely local UI, read `references/patterns/ui-patterns.md` instead.
