...
If the hard part is choosing between `controls`, `playerhud`, `containerwidget`, popup screens, or `frontend`, read `references/patterns/ui-patch-patterns.md`.
If the task also depends on input handlers, read `references/patterns/input-patterns.md`.
If the UI should trigger real gameplay authority, also read `references/patterns/networking-patterns.md` or `references/patterns/action-patterns.md`.
If the task is mostly about local globals such as `ThePlayer` or `TheFrontEnd`, also read `references/patterns/runtime-local-ui.md`.
...
- persistent HUD child, status widget, inventory-adjacent element, or layout patch
  - read `references/patterns/ui-patch-patterns.md`
...
- if the route is unclear, stop and read `references/patterns/ui-patch-patterns.md`
...
If cleanup is the likely failure mode, read `references/pitfalls/ui-pitfalls.md`.
...
- "patch the existing HUD"
  - start with `references/patterns/ui-patch-patterns.md`
...
- "UI duplicates or keeps responding after close"
  - read `references/pitfalls/ui-pitfalls.md`
