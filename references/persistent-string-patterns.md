...
If the task is ordinary entity or world save lifecycle, read `references/persistence-patterns.md` instead.
If the task is user-facing host config in the mod configuration screen, read `references/modinfo-patterns.md` instead.
If the task is mainly about `pcall(...)` or `xpcall(...)` around decode or encode boundaries, also read `references/protected-call-patterns.md`.
...
For the Lua protected-call side of this pattern, also read `references/protected-call-patterns.md`.
